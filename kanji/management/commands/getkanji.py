        ...
        for data in response:
            if not Character.objects.filter(character=data['kanji']['character']).exists():
                character = Character(
                    ...
                    strokes   = functools.reduce(lambda x, y: x + '   '+y, data['kanji']['strokes']['images']),
                    video     = data['kanji']['video']['mp4'],
                )
                character.save()
                self.stdout.write(self.style.SUCCESS('%s has been added to the database' % data['kanji']['character']))
